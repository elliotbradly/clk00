...
server.listen(PORT, async () => {
    ...
    exec('tsc -b 811.orb', async (err, stdout, stderr) => {
        if (err) {
...
const init = async (prt) => {
    ...
    ORB = require(path.resolve('./dist/811.orb/hunt'));
    ORB_ACTION = require(path.resolve('./dist/811.orb/00.orb.unit/orb.action'));
    ...
    if ( pvt == false){
      ...
      await ORB.hunt( ORB_ACTION.INIT_ORB , { val: 1, dat: MQTT, src:  [localBit]  });
